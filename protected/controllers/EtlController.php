...
class EtlController extends Controller
{
	...
	public function actionImpcompact($date=null){
		...
		$query = 'INSERT IGNORE INTO F_Imp_Compact (id, D_Demand_id, D_Supply_id, date_time, D_UserAgent_id, D_GeoLocation_id, unique_id, pubid, ip_forwarded, referer_url, referer_app, imps, ad_req, device_type, device_brand, device_model, os, os_version, browser, browser_version, country, carrier, connection_type ) 
		SELECT i.id, i.tags_id, i.placements_id, i.date, u.id, g.id, SHA(CONCAT(i.server_ip,i.user_agent)), i.pubid, i.ip_forwarded, i.referer, i.app, count(SHA(CONCAT(i.server_ip,i.user_agent))), count(SHA(CONCAT(i.server_ip,i.user_agent))), u.device_type, u.device_brand, u.device_model, u.os, u.os_version, u.browser, u.browser_version, g.country, g.carrier, g.connection_type ) 
		FROM imp_log i 
		INNER JOIN D_UserAgent u   ON(i.user_agent = u.user_agent) 
		INNER JOIN D_GeoLocation g ON(i.server_ip  = g.server_ip) ';
