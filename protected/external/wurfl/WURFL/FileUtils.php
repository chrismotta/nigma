...
class WURFL_FileUtils
{
    ...
    {
        $temp_dir = sys_get_temp_dir() . DIRECTORY_SEPARATOR . 'wurfl_tmp_' . md5(WURFL_Constants::API_VERSION . __DIR__);
        if ((is_dir($temp_dir) && is_writable($temp_dir)) === false && self::mkdir($temp_dir) === false) {
            throw new WURFL_WURFLException("The temp dir could not be created [$temp_dir], check permissions");
        }
        return $temp_dir;
    }
