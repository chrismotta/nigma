...
class WURFL_CustomDevice
{
    ...
    {
        $vc_provider = $this->makeVirtualCapabilityProvider();
        return $vc_provider->get($name);
    }
    
    public function getAllVirtualCapabilities()
    {
        $vc_provider = $this->makeVirtualCapabilityProvider();
        return $vc_provider->getAll();
    }

    private function makeVirtualCapabilityProvider()
    {
        if ($this->virtualCapabilityProvider === null) {
            $device = clone $this;
            $request = clone $this->request;
            $this->virtualCapabilityProvider = new WURFL_VirtualCapabilityProvider($device, $request);
        }
