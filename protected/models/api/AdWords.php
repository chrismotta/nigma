...
class AdWords
{
	...
	public function downloadInfo()
	{
		...
		foreach ($advertisers->entries as $advertiser) {
			...
			echo json_encode($result); echo "<hr>"; return; // FIXME removed

			// process every campaign
			foreach ($result['report']['table']['row'] as $campaign) {
				$dailyReport = new DailyReport();

				var_dump($campaign); echo "<hr>";
				echo json_encode($campaign); echo "<hr>"; continue; // FIXME removed
				...
				$dailyReport->spend = $campaign['cost'] * 1000000; // cost is return in micropound, why? google, why?
